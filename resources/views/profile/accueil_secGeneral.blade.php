@extends('secdg.base')
@section('content')
    @if ($user->autorisation == 0)
        <div class="d-flex justify-content-center align-items-center min-vh-50 py-5">
            <div class="card border-0 shadow-lg rounded-4 text-center px-4 py-5"
                style="max-width: 640px; background: linear-gradient(135deg, #fff5f5 0%, #fff 100%);">
                <div class="mb-3">
                    <span
                ...
                <h3 class="text-danger fw-bold mb-2">Accès refusé</h3>
    ...
    @else
        @php
            // ============================================================
            // Routes par défaut (complétées pour éviter les erreurs
            // si $routes n'est pas transmis par le contrôleur).
// ============================================================
$routes = $routes ?? [
...
// Sécurité : garantir que toutes les clés utilisées existent
$routes += [
    'utilisateurs' => 'les_utilisateurs',
    'employes' => 'les_employes',
    'affectations' => 'les_affectations',
...
// S'assurer que les 12 mois sont présents pour le graphique
            // (robuste même si la collection contient plus ou moins de 12 entrées)
            $presencesParMois = collect($presencesMensuelles->pluck('presences'))->pad(12, 0)->slice(0, 12)->values();

            // Nombre total d'employés (casté en int pour le JS)
            $totalEmployes = (int) $les_employes;
        @endphp
            ...
            <details class="card attendance-chart p-4 mb-4" id="attendanceDetails">
                ...
                <div class="chart-container">
                    <canvas id="attendanceChart"
                        aria-label="Histogramme des présences mensuelles comparées au nombre d’employés"
                        role="img"></canvas>
                </div>
            </details>
        ...
        @push('scripts')
            <script>
                document.addEventListener('DOMContentLoaded', function() {
                    ...
                    const employeeTotal = @json($totalEmployes);
